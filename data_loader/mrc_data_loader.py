...
class MRCNERDataLoader(object):
    ...
    def get_dataloader(self, data_sign="train"):
        ...
        #contexts = torch.tensor([f.contexts for f in features], dtype=torch.long)
        contexts = list([f.contexts for f in features])
        #print(contexts)
        ...
        if data_sign == "train":
            ...
            dataloader = DataLoader(dataset, sampler=datasampler, batch_size=self.train_batch_size, num_workers=200)
        elif data_sign == "dev":
            datasampler = SequentialSampler(dataset)
#            print(datasampler)
            dataloader = DataLoader(dataset, sampler=datasampler, batch_size=self.dev_batch_size, num_workers=200)
        elif data_sign == "test":
            datasampler = SequentialSampler(dataset)
#            print(dataset) 
            dataloader = DataLoader(dataset, sampler=datasampler, batch_size=self.test_batch_size, num_workers=200)
            dataloader = (dataloader,contexts)
        return dataloader
