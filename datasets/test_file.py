...
class MRCNERDataset(Dataset):
    ...
    def __getitem__(self, item):
        ...
        #     if end+1 > list(origin_offset2token_idx_end.keys())[-1]:
        #         print('end is here')
        #         print(end)
        #         temp_end = end_positions[:-1]
        #         temp_end.append(list(origin_offset2token_idx_end.keys())[-1]-1)
        # if temp_end:
        #     print('this is temp end')
        #     print(temp_end)
        #     end_positions = temp_end
        for start in start_positions:
            if start not in origin_offset2token_idx_start.keys():
                j = (max(key for key in map(int, origin_offset2token_idx_start.keys()) if key <= start))
                new_start_positions.append(origin_offset2token_idx_start[j])
                #print(j)
            else:
        ...
        for end in end_positions:
            #print(end)
            if end not in origin_offset2token_idx_end.keys():
                j = (max(key for key in map(int, origin_offset2token_idx_end.keys()) if key <= end))
                #print(j)
                new_end_positions.append(origin_offset2token_idx_end[j])
            else:
                new_end_positions.append(origin_offset2token_idx_end[end])
                #print(end)
        #new_start_positions = [origin_offset2token_idx_start[start+1] for start in start_positions]
        #new_end_positions = [origin_offset2token_idx_end[end+1] for end in end_positions]
        label_mask = [
...
def run_dataset():
    ...
    json_path = "/home2/shravya.k/sebi/mrc-ner.train"
    is_chinese = False
